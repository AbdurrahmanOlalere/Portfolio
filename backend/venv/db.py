...
def get_connection(database=None):
    return psycopg2.connect(
        host=rds.host,
        port=rds.port,
        user=rds.user,           # ← svc_portfolio_rw
        password=rds.password,   # ← RW password
...
def insert_details(name, email, message):
    with get_connection() as conn:
        with conn.cursor() as cursor:
            insert_query = """
                INSERT INTO public.details (name, email, message)
                VALUES (%s, %s, %s)
            """
...
def get_details():
    with get_connection() as conn:
        with conn.cursor() as cursor:
            cursor.execute("SELECT * FROM public.details ORDER BY id DESC;")
